...
import edu.ufp.inf.sd.rabbitmqservices.projeto.jogo.units.Base;
...
public class ComputerBrain {
	...
	public void ThinkDamnYou(edu.ufp.inf.sd.rabbitmqservices.projeto.jogo.players.Base ply) {
		...
		if (!DoneUnits) {
			...
			edu.ufp.inf.sd.rabbitmqservices.projeto.jogo.buildings.Base bld = Game.builds.get(CityCount);
			CityCount++;
	...
	private void HandleBuilding(edu.ufp.inf.sd.rabbitmqservices.projeto.jogo.buildings.Base bld) {
		if (!bld.Menu.equals("")) {
			if (!bld.Locked) {
	...
	/**Looks for any enemies in range at the edu.ufp.inf.sd.rabbitmqservices.projeto.jogo.units current location by attacking each location in the attackable locations.*/
