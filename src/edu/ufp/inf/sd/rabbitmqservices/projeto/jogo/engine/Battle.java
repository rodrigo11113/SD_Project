...
public class Battle {
	/**A count of all the edu.ufp.inf.sd.rabbitmqservices.projeto.jogo.players in the game, used before Game.player is populated so this is required.*/
	...
	public void NewGame(String mapname) {
		Game.player = new ArrayList<edu.ufp.inf.sd.rabbitmqservices.projeto.jogo.players.Base>();
		Game.builds = new ArrayList<edu.ufp.inf.sd.rabbitmqservices.projeto.jogo.buildings.Base>();
	...
	public void EndTurn() {
		edu.ufp.inf.sd.rabbitmqservices.projeto.jogo.players.Base ply = Game.player.get(currentplayer);
	...
	/**Grabs the number of edu.ufp.inf.sd.rabbitmqservices.projeto.jogo.buildings a player owns.*/
	...
	public void MaxUsers(int max) {
		//Setup for max edu.ufp.inf.sd.rabbitmqservices.projeto.jogo.players
		if (max<2) {totalplayers = 2;}
		else if (max>12) {totalplayers = 12;}
		else {totalplayers = max;}
		//HACK: Change when I add more images to support more edu.ufp.inf.sd.rabbitmqservices.projeto.jogo.players.
		if (max>4) {totalplayers = 4;
			Game.error.ShowError("The game currently supports only 4 edu.ufp.inf.sd.rabbitmqservices.projeto.jogo.players, not " + max + ".");
		}
	}
	...
	public void CaptureCapital(int x, int y) {
		...
		for (int i = 0; i < Game.builds.size(); i++) {
			if (Game.builds.get(i).x == x && Game.builds.get(i).y == y) {
				...
				if (playersleft<=1) {
					GameOver = true;
					new edu.ufp.inf.sd.rabbitmqservices.projeto.jogo.menus.EndBattle();
				}
				break;
			}
		}
		...
		//TODO: Change all edu.ufp.inf.sd.rabbitmqservices.projeto.jogo.buildings to be owned by the player.
	}
