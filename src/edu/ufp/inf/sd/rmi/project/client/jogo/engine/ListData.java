 ...
 * For finding type when Loading, it takes the edu.ufp.inf.sd.rmi.project.client.jogo.edu.ufp.inf.sd.rabbitmqservices.projeto.jogo.edu.ufp.inf.sd.rabbitmqservices.projeto.jogo.buildings.edu.ufp.inf.sd.rabbitmqservices.projeto.jogo.units name, and compares it to the Display# list (reference list).
...
public class ListData {
	 ...
	 */
	public edu.ufp.inf.sd.rmi.project.client.jogo.players.Base CreateCommander(int which, int team, int money, boolean npc) {
		switch (which) {
			case 0:return new edu.ufp.inf.sd.rmi.project.client.jogo.players.Andy(npc,team,money);
			case 1:return new edu.ufp.inf.sd.rmi.project.client.jogo.players.Colin(npc,team,money);
			case 2:return new edu.ufp.inf.sd.rmi.project.client.jogo.players.Max(npc,team,money);
			case 3:return new edu.ufp.inf.sd.rmi.project.client.jogo.players.Sammi(npc,team,money);
		}
		return null;
	}
	/**
	 * This creates a building (town/barracks/seaport) [MapParser]
	 * @param owner = The player that owns the building (-1 to skip formatting) (15 is neutral, 12~14 are unused (and more if I lower max edu.ufp.inf.sd.rmi.project.client.jogo.edu.ufp.inf.sd.rabbitmqservices.projeto.jogo.edu.ufp.inf.sd.rabbitmqservices.projeto.jogo.buildings.edu.ufp.inf.sd.rabbitmqservices.projeto.jogo.players from 12))
	 * @param xx
	 * @param yy
	...
	public void LoadCommander(String Name, boolean Defeated, int Team, int Money, int Kills, int Loses, int Power, int Using, boolean npc) {
		for (int i = 0; i < Game.displayC.size(); i++) {
			if (Game.displayC.get(i).name.equals(Name)) {
				...
				edu.ufp.inf.sd.rmi.project.client.jogo.players.Base ply = Game.player.get(Game.player.size()-1);
	...
	public void LoadCity(String Name, int Owner, int Health, int id) {
		...
		edu.ufp.inf.sd.rmi.project.client.jogo.buildings.Base bld = Game.builds.get(id);
		bld.owner = Owner;
		bld.health = Health;
	}
	...
	public void LoadUnit(String Name, int Owner, int Health, int Ammo, int Fuel, int X, int Y, boolean Acted) {
		for (int i = 0; i < Game.displayU.size(); i++) {
			if (Game.displayU.get(i).name.equals(Name)) {
				...
				edu.ufp.inf.sd.rmi.project.client.jogo.units.Base unit = Game.units.get(Game.units.size()-1);
				unit.health = Health;
				unit.Ammo = Ammo;
				unit.Fuel = Fuel;
			}
